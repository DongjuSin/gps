...
class AgentBaxter(Agent):
    ...
    def _init_sample(self, condition, feature_fn=None):
        ...
        if RGB_IMAGE in self.obs_data_types:
            ...
            np.savez('camera_image_blind_' + str(condition) + '.npz', img=self.img)
        ...
        if RGB_IMAGE in self.obs_data_types:
            ...
            if feature_fn is not None:
                ...
                if t == 18:
                    ...
                    now = str(today.month) + '_' + str(today.day) + '_' + str(today.hour) + '_' + \
                            str(today.minute) + '_' + str(today.second)
                    np.savez('./trajectory/data/final_obs_' + now + '.npz', obs=obs)
