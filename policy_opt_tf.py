...
class PolicyOptTf(PolicyOpt):
    ...
    def __init__(self, hyperparams, dO, dU):
        ...
        self.policy = TfPolicy(dU, self.obs_tensor, self.act_op, self.feat_op,
                               np.zeros(dU), self.sess, self.device_string, copy_param_scope=self._hyperparams['copy_param_scope'],
                               conv_layer_0=self.conv_layer_0, conv_layer_1=self.conv_layer_1, conv_layer_2=self.conv_layer_2)
        ##
    ...
    def update(self, obs, tgt_mu, tgt_prc, tgt_wt):
        ...
        """
        # np.savez('./policy_update/policy_update.npz', obs=obs, tgt_mu=tgt_mu, tgt_prc=tgt_prc, tgt_wt=tgt_wt)
        N, T = obs.shape[:2]
        ...
        # actual training.
        #self._hyperparams['iterations'] = 3000
        #self._hyperparams['iterations'] = 1000
        for i in range(self._hyperparams['iterations']):
            ...
            feed_dict = {self.obs_tensor: obs[idx_i],
                         self.action_tensor: tgt_mu[idx_i],
                         self.precision_tensor: tgt_prc[idx_i]}
            print "obs shape: ", obs[idx_i].shape
            print "action shape: ", tgt_mu[idx_i].shape
            print "precision shape: ", tgt_prc[idx_i].shape
    ...
    def __getstate__(self):
        ...
        check_path = self.saver.save(self.sess, check_file, write_meta_graph=False)
        LOGGER.debug('Saving model to: %s', check_path)
        print('Saving model to: ' + check_path)
        # checkpoint_path = self.checkpoint_file
        # check_path = self.saver.save(self.sess, checkpoint_path, write_meta_graph=False)
        # LOGGER.debug('Saving model to: %s', checkpoint_path)
    ...
    def __setstate__(self, state):
        ...
        check_file = '/'.join(str.split(self.checkpoint_file, '/')[:-1])
        print('check_file: ' + check_file)
        check_path = self.saver.restore(self.sess, tf.train.latest_checkpoint(check_file))
        # print('Restoring model from: ' + check_path)
        '''
