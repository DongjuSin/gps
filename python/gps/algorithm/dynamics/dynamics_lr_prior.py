...
class DynamicsLRPrior(Dynamics):
    ...
    def save_fitted_dynamics(self, X, U, cond, itr):
        import os.path

        path = '/hdd/gps-master/plot_coeff/itr_' + str(itr)
        if os.path.exists(path) == False:
            os.mkdir(path)

        np.savez(path + '/' + 'cond_' + str(cond) + '.npz', Fm=self.Fm, fv=self.fv, dyn_covar=self.dyn_covar, X=X, U=U)
